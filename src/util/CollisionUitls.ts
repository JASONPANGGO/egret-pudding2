...
namespace util {
    export class CollisionUitls {
        ...
        public static checkPolygon(r1PointList: IPoint[], r2PointList: IPoint[]): boolean {
            ...
            const fun1 = function (pointlist: IPoint[], vertexPoints1: IPoint[], vertexPoints2: IPoint[]): boolean {
                //先计算 r1的边 ---》 顺时针
         ...
         * 检测两个对象外围矩形框是否相交
         */
        public static checktObjPolygon(obj1: egret.DisplayObject | egret.DisplayObjectContainer, obj2: egret.DisplayObject | egret.DisplayObjectContainer): boolean {
            let obj1PointList = this.getRectangleVerPoints(obj1);
            let obj2PointList = this.getRectangleVerPoints(obj2);
            obj1PointList = this.switchGlobalPoint(obj1, obj1PointList);
            obj2PointList = this.switchGlobalPoint(obj2, obj2PointList);
            return this.checkPolygon(obj1PointList, obj2PointList);
        }

        /** 转换成一组世界坐标 */
        private static switchGlobalPoint(obj: egret.DisplayObject | egret.DisplayObjectContainer, objPoints: IPoint[]): IPoint[] {
            let pos: egret.Point;
            for (let i = 0; i < objPoints.length; i++) {
                const point = objPoints[i];
                pos = gComMgr.toGlobal(obj, point.x, point.y, pos);
                if (pos) {
                    objPoints[i].x = pos.x;
                    objPoints[i].y = pos.y;
                }
