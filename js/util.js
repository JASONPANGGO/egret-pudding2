...
; (function (win) {
  ...
  mv_utils.test = function () {
    console.log('test');
  }
  /*获取URL路径参数*/
  mv_utils.getUrlParams = function (url) {
    var i;
  ...
  /*把一个http域名数组换成对应的https域名数组*/
  mv_utils.http2https = function (adUnits) {
    var httpsMap = {
  ...
  /*调用WindVane方法*/
  mv_utils.hybirdEvent = function (opt) {
  ...
  /*使用JS原生方法获取id元素或者class元素*/
  mv_utils.getEl = function (el, tagName) {
    el = el.replace(/^\s|\s$/g, '');
    // # id . class tag
  ...
  /*使用JS原生方法删除元素节点，如果这个节点有父节点的话 */
  mv_utils.rmEl = function (el) {
  ...
  mv_utils.addClass = function (el, className) {
  ...
  mv_utils.rmClass = function (el, className) {
    var reg = new RegExp('' + className + '');
    if (reg.test(el.className)) {
  ...
  mv_utils.ajax = function (type, url, data, success, failed) {
    // 创建ajax对象
    var xhr = null;
  ...
  mv_utils.statistics = function (url, silentTag) {
    var reg = /(http\:|https\:)/;
  ...
  // 文档高度
  mv_utils.getDocumentTop = function () {
  ...
  // 可视窗口高度
  mv_utils.getWindowHeight = function () {
    var windowHeight = 0;
  ...
  // 可视窗口宽度
  mv_utils.getWindowWidth = function () {
    var windowWidth = 0;
    if (document.compatMode == "CSS1Compat") {
  ...
  // 滚动条滚动高度
  mv_utils.getScrollHeight = function () {
  ...
  mv_utils.toggleLogo = function () {
    var i;
    var parm = mv_utils.getUrlParams();
    var rVersion = /(mal|mi)/ig;
    if (parm && parm.sdk_version) {
      if (rVersion.test(parm.sdk_version)) {
        for (i = 0; i < mv_utils.getEl('.logo').length; i++) {
          mv_utils.rmClass(mv_utils.getEl('.logo')[i], 'none');
        }
      }
    }
  }

  mv_utils.gpStats = function (cb, ua, time) {
	 ...
	 * utf16 = utf8to16(utf8);
	 */
  mv_utils.utf16to8 = function (str) {
    var out, i, len, c;
    out = "";
    len = str.length;
    for (i = 0; i < len; i++) {
  ...
  mv_utils.utf8to16 = function (str) {
    var out, i, len, c;
    var char2, char3;
    out = "";
    len = str.length;
  ...
  mv_utils.clearConsole = function () {
    var method;
    var noop = function () { };
    var methods = [
